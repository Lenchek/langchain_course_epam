"""Write confirmed reservations to a text file. Format: Name | Car Number | Reservation Period | Approval Time.

Used by the FastAPI server and as fallback when server is not running (tool/function call).
"""
...
def write_confirmed_reservation(
    ...
    """Append one line to the confirmed reservations file. Returns the file path."""
...
def notify_confirmed_reservation(req: dict, approval_time: str) -> None:
    """On admin approve: send to reservation server (if URL set) or write to file directly (function call)."""
