"""RAG chatbot (first agent) + escalation to admin (second agent) for reservation approval."""
import re
...
# ---------- Stage 2: escalation to administrator ----------

RESERVATION_FIELDS = ["name", "surname", "car_number", "period_start", "period_end"]


def extract_reservation_from_text(text: str) -> dict:
    """Use LLM to extract reservation fields from user message. Returns dict with keys in RESERVATION_FIELDS or empty."""
    prompt = PromptTemplate(
...
def try_submit_reservation(user_message: str) -> tuple:
    """
    If user_message contains reservation details, create request and return (True, reply_with_request_id).
...
def answer_reservation_status(user_message: str) -> Optional[str]:
    """If user asks for status of request ID, return status message; else None."""
